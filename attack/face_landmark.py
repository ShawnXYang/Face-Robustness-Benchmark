...
#f = open("./data/cfp_aligned_landmarks.json", "r")
f = open("./data/alignedAll_landmarks.json", "r")
landmark_values = json.load(f)

def get_landmark(name):
    pts_img = np.array(landmark_values[name])
    
    heatmaps = np.zeros((112, 112), dtype=np.float32)
    for i in range(17,68):
        if pts_img[i, 0] > 0 and pts_img[i, 0] < 112 and pts_img[i, 1] < 112:
            heatmaps = draw_gaussian(heatmaps, pts_img[i], 2)
    heatmaps = np.tile(heatmaps[..., np.newaxis], (1, 1, 3))
    return heatmaps

def get_sample_landmark(name, num_samples = 4, sigma=1):
    pts_img = []
...
def getlist_landmark(names, num_samples=4, img_shape=(112,112), sigma=1):
    lands = np.zeros(shape = (len(names),) + img_shape + (3,), dtype=np.float32)
    for idx, name in enumerate(names):
        land = get_sample_landmark(name, num_samples, sigma)
