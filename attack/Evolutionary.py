...
class Evolutionary(Attack):
    ...
    def batch_attack(self, xs, ys, pairs, **kwargs):
        ...
        for i in range(xs.size(0)):
            path = pairs[i][1]
            x_target = imread(path).astype(np.float32)
            x_target = torch.Tensor(x_target.transpose((2, 0, 1))[None, :]).cuda()
            y = self.model.forward(x_target)
            adv[i] = self.attack(xs[None, i], ys[i], x_target, self.model)
        return adv
