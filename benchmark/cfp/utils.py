...
def read_pairs(datadir, goal, model, batch_size):
    """
        Provide an iterable for CFP dataset.
        Args:
            datadir: The path of CFP dataset
            goal: impersonate or dodging
            model: A FaceModel
            batch_size: How many samples per batch to load
    """
    pairs = []
    cfp_dir = '/data/dingcheng/cfp-dataset/'
    prefix = os.path.join(cfp_dir,'Protocol/')
    ...
    prefix = os.path.join(cfp_dir,"Protocol/Split/FP")
    ...
    for folder in folders_1:
        ...
        with open(img_root_path, 'r') as f:
            for line in f.readlines()[0:]:
                ...
                path1 = os.path.join(datadir, id1, path1, filename1)
                xs.append(read_pair(path1, model)[0])
                id2, path2, filename2 = pairs_P[int(pair1[1])-1].split('/')[-3:]
                path2 = os.path.join(datadir, id2, path2, filename2)
                ys.append(read_pair(path2, model)[1])
...
def run_white(path, Attacker, model, args, threshold):
    ...
    advs = []
    imgs = []
    for xs, ys, pairs in tqdm(read_pairs(path, args.goal, model, args.batch_size), total=3500//args.batch_size):
    ...
    with open(os.path.join('log-cfp', args.log), 'w') as f:
