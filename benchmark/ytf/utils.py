...
def read_pairs(datadir, goal, model, batch_size):
    """
        Provide an iterable for YTF dataset.
        Args:
            datadir: The path of YTF dataset
            goal: impersonate or dodging
            model: A FaceModel
            batch_size: How many samples per batch to load
    """
    with open(os.path.join('data', 'splits.txt')) as f:
        lines = f.readlines() 
    suffix = '.jpg'
    xs, ys, pairs = [], [], []
    idx = 0
    for line in lines:
        ...
        if (int(line[-1]) == 1 and goal == 'dodging') or (int(line[-1]) == 0 and goal == 'impersonate'):
            ...
            path = os.path.join(datadir, name, name + '_' + pid + '.png')
            pair.append(path)
...
def run_white(path, Attacker, model, args, threshold):
    ...
    advs = []
    imgs = []
    for xs, ys, pairs in tqdm(read_pairs(path, args.goal, model, args.batch_size), total=2500//args.batch_size):
